...
class Player:
    ...
    def draw(self, surface):
        ...
        if self.down_key_timer > 0.0 and self.current_platform:
            ...
            # Red/Orange for dark gray step (damage), Green for green step (safe)
            bar_color = (255, 100, 100) if self.current_platform.type == "normal" else (100, 255, 100)
            ...
            if state == "PLAYING":
                ...
                if frame_count >= PLATFORM_SPAWN_RATE:
                    ...
                    if r < 0.60:
                        ...
                        p_type = "purple"
                    platforms.append(Platform(p_x, HEIGHT + 20, p_type))
                    ...
                    if (player.vel_y > 0 if p.type == "spike" else player.vel_y >= -p_speed) and p.rect.top <= player.rect.bottom <= p.rect.top + 20 and \
                       player.rect.right > p.rect.left and player.rect.left < p.rect.right:
                        ...
                        if p.type == "spike":
                            ...
                            if is_new_landing:
                                # Green stair: count for combo and score (+2, x2 in rainbow)
                            ...
                            if is_new_landing:
                                # Dark gray (normal) stair: count for combo and score
                                player.combo_green_blue += 1
                ...
                if player.current_platform and player.current_platform.type in ["normal", "heal"]:
